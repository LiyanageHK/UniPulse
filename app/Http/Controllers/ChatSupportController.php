...
class ChatSupportController extends Controller
{
    ...
    public function deleteConversation(Request $request, $id)
    {
        ...
        // Prevent deletion of conversations with crisis flags
        if ($conversation->hasCrisisFlags()) {
            return response()->json([
                'success' => false,
                'error' => 'Cannot delete conversations flagged for counselor review',
            ], 400);
        }
        ...
        try {
            // 1. Delete all embeddings related to this conversation
            $deletedData['embeddings'] = ConversationEmbedding::where('conversation_id', $conversation->id)->delete();

            // 2. Delete all memories sourced from this conversation
            $deletedData['memories'] = Memory::where('source_conversation_id', $conversation->id)->delete();

            // 3. Delete all messages (will also remove message-specific embeddings and memories via cascade)
